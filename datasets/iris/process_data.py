...
df = pd.read_csv(filename,names=['s-len','s-width','p-len','p-width','iris'])
df['iris']=df['iris'].astype('category').cat.codes
x = df.drop(labels='iris',axis=1).values
y = df['iris'].values
n = x.shape[0]
...
# NOTE: Wu method only compatible with classification datasets with two outputs -- incompatible with iris data
